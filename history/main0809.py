...
class Example(QWidget):  # TODO 主窗口类
    ...
    def Runcar(self):  # * 运行函数
        global  boardmap, True_Treas_Num, treasureinmap, ifhit, hitdistance, hitdirection, TreasureIfo, NextTreasure, Trea_distances, cropimg
        # 数据初始化
        True_Treas_Num = 0
        ifhit = 1
        CurrentTreasure = 0
...
class Example(QWidget):  # TODO 主窗口类
    ...
    def Runcar(self):  # * 运行函数
        ...
        for itel in range(10):
            ...
            if TreasureIfo[NextTreasure][4] == 0 and TreasureIfo[NextTreasure][6] == 0:
                camera = Camera(1) # 打开相机
                camera.open()
                start_time = time.time()
                while time.time() - start_time < 0.5:
