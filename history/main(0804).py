...
def CreateStep(itel, final, false): #* 创建步进函数
    ...
    if (final == 1): # 如果final为1，进入最后一段路径
    ...
    elif (false == 1):# 如果false为1，则进入不碰撞快速通过模式
        # 设置一个新的坐标，将宝藏点坐标存入其中
        ...
        print("next_treasure:", treasureinmap[itel][0], treasureinmap[itel][1])
        result_this_time = astar(map)
        result_this_time.reverse()
...
class Example(QWidget): #TODO 主窗口类
    ...
    def initUI(self): #* 界面初始化函数
        ...
        self.btn.setStyleSheet("QPushButton:hover{color:black}"  # 光标移动到上面后的前景色
                                  ...
                                  "QPushButton{padding:2px 4px}"  # 按键内边界
                                  "QPushButton{font-family:'黑体';font-size:30px}")  # 字体样式
        self.setGeometry(1200, 500, 600, 400)
    ...
    def Runcar(self): #* 运行函数
        global result_final, boardmap, True_Treas_Num, treasureinmap, ifhit, hitdistance, hitdirection
        # 数据初始化
        True_Treas_Num = 0
        ifhit = 1
        ...
        for itel in range(len(result_final)):
            ...
            if ifhit == 1: # 如果上次击打了
                # TODO 计算地图
                if True_Treas_Num == 3: # 如果已经找到3个宝藏，那么就计算从当前位置到终点的路径
            ...
            # TODO 发送撞击指令与否
            # 如果是蓝色方，且为真宝藏，发送撞击指令
            if set_team_color == 1 and Treasure == "BlueTrue":
                ifhit = 1
                True_Treas_Num += 1
            else:
                ifhit = 0
            ...
            # 如果是红色方，且为真宝藏，发送撞击指令
            if set_team_color == 0 and Treasure == "RedTrue":
                ifhit = 1
                True_Treas_Num += 1
            else:
                ifhit = 0
