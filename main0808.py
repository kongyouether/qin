...
def CreateStep(round, start, end, false):  # * 创建步进函数
    ...
    global result_final, boardmap, True_Treas_Num, treasureinmap, hitdirection, hitdistance, boardx
...
class Example(QWidget):  # TODO 主窗口类
    ...
    def Startup(self):  # * 启动函数
        global result_final, boardmap, map_gui_image, cell, map_cv2_image, treasureinmap, TreasureIfo, Trea_distances, cropimg
        ...
        # cv2.imwrite('./reshapemap.jpg', img)  # 保存图片
        # 将图像显示在QT控件中
        ...
        for i in range(len(boardmap)):  # 将boardmap每行依次打印
            print(boardmap[i])
        # 将图像显示在QT控件中
    ...
    def Runcar(self):  # * 运行函数
        global boardmap, True_Treas_Num, treasureinmap, ifhit, hitdistance, hitdirection, TreasureIfo, NextTreasure, Trea_distances, cropimg, cell, result_final, CurrentTreasure
        # 数据初始化
        True_Treas_Num = 0
        ifhit = 1
        CurrentTreasure = 0
        ...
        for itel in range(10):
            ...
            if TreasureIfo[NextTreasure][4] == 0 or TreasureIfo[NextTreasure][4] == 1 or TreasureIfo[NextTreasure][4] == 2:
                # camera = Camera(1)  # 打开相机
                # camera.open()
                # start_time = time.time()
                ...
                Treas_image, contours_blue, blue = Identify.FindColorOne(Treas_img_copy, 0)  # 蓝色
                Treas_image, contours_red, red = Identify.FindColorOne(Treas_img_copy, 3)  # 红色
            ...
            for i in range(8):
                if TreasureIfo[i][4] == 1: # 红色宝藏
                    cv2.circle(cropimg, (cell * TreasureIfo[i][0] + int(cell * 3), cell * TreasureIfo[i][1] + int(cell * 1)), 10, (255, 0, 0), -1)
                elif TreasureIfo[i][4] == 2: # 蓝色宝藏
                    cv2.circle(cropimg, (cell * TreasureIfo[i][0] + int(cell * 3), cell * TreasureIfo[i][1] + int(cell * 1)), 10, (0, 0, 255), -1)
                elif TreasureIfo[i][4] == 3: # 红色真宝藏
                    cv2.circle(cropimg,(cell * TreasureIfo[i][0] + int(cell * 3), cell * TreasureIfo[i][1] + int(cell * 1)), 10, (255, 0, 0), -1)
                    cv2.circle(cropimg,(cell * TreasureIfo[i][0] + int(cell * 3), cell * TreasureIfo[i][1] + int(cell * 1)), 5, (0, 255, 0), -1)
                elif TreasureIfo[i][4] == 4: # 红色假宝藏
                     cv2.circle(cropimg, (cell * TreasureIfo[i][0] + int(cell * 3), cell * TreasureIfo[i][1] + int(cell * 1)), 10, (255, 0, 0), -1)
                     cv2.circle(cropimg, (cell * TreasureIfo[i][0] + int(cell * 3), cell * TreasureIfo[i][1] + int(cell * 1)), 5, (255, 255, 0), -1)
                elif TreasureIfo[i][4] == 5: # 蓝色真宝藏
                    cv2.circle(cropimg, (cell * TreasureIfo[i][0] + int(cell * 3), cell * TreasureIfo[i][1] + int(cell * 1)), 10, (0, 0, 255), -1)
                    cv2.circle(cropimg, (cell * TreasureIfo[i][0] + int(cell * 3), cell * TreasureIfo[i][1] + int(cell * 1)), 5, (255, 255, 0), -1)
                elif TreasureIfo[i][4] == 6: # 蓝色假宝藏
                    cv2.circle(cropimg, (cell * TreasureIfo[i][0] + int(cell * 3), cell * TreasureIfo[i][1] + int(cell * 1)), 10, (0, 0, 255), -1)
                    cv2.circle(cropimg, (cell * TreasureIfo[i][0] + int(cell * 3), cell * TreasureIfo[i][1] + int(cell * 1)), 5, (0, 255, 0), -1)
